...
function onOpen() {
  ...
  ui.createMenu('🐛デバッグ')
    .addItem('本日のカレンダーイベントを表示', 'debugShowToday')
    .addItem('2026-03-01のカレンダーイベントを表示', 'debugShowMarch')
    .addToUi();
}
...
/**
 * 本日のカレンダーイベントをすべて列挙（デバッグ用）
 * GAS コンソール或いはメニューから実行
 */
function debugShowToday() {
  try {
    const today = new Date();
    const dateStr = Utilities.formatDate(today, 'Asia/Tokyo', 'yyyy-MM-dd');
    debugShowCalendarEventsForDate(dateStr);
    
    const ui = getUiSafe_();
    if (ui) {
      ui.alert('✅ デバッグ完了', 'コンソール (Ctrl+Shift+J) でログを確認してください', ui.ButtonSet.OK);
    }
  } catch (e) {
    console.error('debugShowToday error:', e.message);
    const ui = getUiSafe_();
    if (ui) {
...
/**
 * 2026-03-01 のカレンダーイベントをすべて列挙（デバッグ用）
 * 前回の同期テスト対象日
 */
function debugShowMarch() {
  try {
    debugShowCalendarEventsForDate('2026-03-01');
    ...
    console.error('debugShowMarch error:', e.message);
    const ui = getUiSafe_();
    if (ui) {
      ui.alert('❌ エラー', e.toString(), ui.ButtonSet.OK);
    }
